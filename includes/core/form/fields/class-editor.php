...
class Editor extends Field {
	...
	protected function prepare() {
		...
		$this->sanitize_cb = array_merge( [ 'description' ], $this->sanitize_cb );
	}
	...
	public function field_markup() {
		...
		$settings = array(
			'textarea_rows' => 10,
			'tinymce'       => array(
				'content_css'      => ANSPRESS_URL . 'editor.css',
	...
	public function image_process( $matches ) {
		...
		if ( ! file_exists( $temp_file ) ) {
			...
			if ( in_array( $basename, $files, true ) ) {
				...
				$img_tag = '<img src="' . esc_url( $new_file_url ) . '"';
				if ( $img['width'] ) {
